...
let aiChats = {};
let requestQueue = {}; // chat-wise queue

async function processQueue(from) {
  if (!requestQueue[from] || requestQueue[from].length === 0) return;

  const { m, body } = requestQueue[from].shift(); // get first request
  try {
    ...
    const response = await axios.post(
      'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=55ba0f3355fea54b6a032e8c5249c60f',
      {
    ...
    setTimeout(() => processQueue(from), 3000);
  }
}
...
cmd({
  ...
  if (!aiChats[from] || isOwner) return;

  if (!requestQueue[from]) requestQueue[from] = [];
  requestQueue[from].push({ m, body });

  // if queue length is 1, start processing
  if (requestQueue[from].length === 1) processQueue(from);
});
